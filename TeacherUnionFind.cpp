...
int discret[SIZE], dn;
struct Union_Find {
    int d[UnionFindSIZE], num[UnionFindSIZE];// d is the pointer, initializing
    void init(int n){
        for(int i = 0; i < n; i++) {
            d[i] = i;   // 點 i 指向 i
...
void solve() {
    ...
    scanf("%d%d", &x, &n);
    discret[dn++] = 0;
    discret[dn++] = x;// discrete 1 = x, which means that we imagine there is a traffic light at the end of the road. 
    for(int i = 1; i <= n; i++) {
        scanf("%d", &p[i]);
        discret[dn++] = p[i];
    }
